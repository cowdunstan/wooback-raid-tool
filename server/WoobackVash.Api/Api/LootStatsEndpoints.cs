...
public static class LootStatsEndpoints
{
    ...
    public static void MapLootStatsEndpoints(this IEndpointRouteBuilder app)
    {
        app.MapGet("/api/loot/stats", async (HttpContext ctx, SessionTokenService tokens) =>
        {
            ...
            // that never asks sees exactly what it always has. Phase is the shared LootPhases
            // classifier (the loot sheets' baked item sets); an item neither set carries shows
            // only in the combined "all" view.
            var phase = ctx.Request.Query["phase"].ToString().Trim().ToLowerInvariant();
            if (phase is "p2" or "p3")
                awards = awards.Where(a => string.Equals(LootPhases.Of(a.ItemName), phase, StringComparison.OrdinalIgnoreCase)).ToList();
